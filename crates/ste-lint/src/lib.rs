...
fn collect_diagnostics(
    ...
    diagnostics.extend(passes::length::check(text, context, mode));
    diagnostics.extend(passes::lists::check(text));
    diagnostics.extend(passes::notes::check(&analysis));
    diagnostics.extend(passes::paragraph::check(text, context, mode));
    diagnostics.extend(passes::procedural::check(&analysis));
    diagnostics.extend(passes::punctuation::check(text, context));
    diagnostics
}

fn outcome(diagnostics: &[Diagnostic]) -> Outcome {
    if diagnostics
        .iter()
