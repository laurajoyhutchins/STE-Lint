...
pub enum ShadowEvidenceError {
    ...
    InvalidConfidence { kind: String },
}

impl fmt::Display for ShadowEvidenceError {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Self::InvalidJson(message) => write!(formatter, "invalid shadow evidence JSON: {message}"),
            Self::UnsupportedSchema(version) => {
                write!(formatter, "unsupported shadow evidence schema version {version}")
            }
            Self::InvalidIdentity(field) => {
                write!(formatter, "invalid or missing shadow evidence identity field {field}")
            }
            ...
            Self::SourceDigestMismatch => {
                write!(formatter, "shadow evidence source SHA-256 does not match the analysis text")
            }
