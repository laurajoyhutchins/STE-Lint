...
fn audited_coverage_downgrades_unsupported_rule_claims() {
    ...
    for id in ["1.4", "3.1", "9.3"] {
        assert_eq!(rules[id]["status"], "not_implemented", "Rule {id}");
        assert_eq!(rules[id]["diagnostic_codes"].as_array().unwrap().len(), 0);
        assert_eq!(rules[id]["evidence_artifacts"].as_array().unwrap().len(), 0);
    }
    ...
    assert_eq!(counts.get("partial"), Some(&34));
    assert_eq!(counts.get("context_required"), Some(&14));
